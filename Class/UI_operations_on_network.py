...
def show_behavioral_space(filename_be, type=0):
    behavioral_layout = [[]]
    if type == 0:
...
def show_obs_space(obs, filename_obs):
    obs_name = ""
    for o in obs:
        obs_name = obs_name + o + " "
...
def show_re_obs_space(obs, filename_re_obs):
    column_re_ob1 = [
        [sg.Image(
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED:
            ...
            show_renomination_file_obs(obs, filename_re_obs)


def show_renomination_file_obs(obs, filename_re_obs):
    file = open(
        file="Output/Behavioral_Space_Observable_Renominated/RL_" + filename_re_obs + ".txt")
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED:
            ...
            show_re_obs_space(obs, filename_re_obs)
